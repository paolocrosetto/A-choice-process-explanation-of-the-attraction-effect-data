...
# installing cmdstanr if not done already
...
# make sure to install mingw32-make and g++ if not done already
...
table_share  <- first %>% right_join(second, by = c("firstclick"), multiple = "all")
...
table_times <- first %>% right_join(second, by = c("firstclick"), multiple = "all")
...
table %>% 
  ...
  kable("latex", booktabs = T, col.names = NULL, valign = "top") %>% 
  add_header_above(c("choice", "share %", "time", "choice", "share %", "time", "choice", "share %")) %>% 
