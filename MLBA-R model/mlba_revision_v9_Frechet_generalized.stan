...
// log likelihood
    ...
    real lba_log(matrix RT, real k, real I, real m, real lam, real gamma, real beta, real stay, real tau, real alpha, data real[] x_r){
          ...
          for (i in 1:rows(RT)){
                    ...
                    prob[i] = lmba_pdf(RT[i,9:14], RT[i,1], c1, RT[i,3], c2, type, k, I, m, lam, gamma, beta, stay, tau, alpha, x_r);
...
model {
    ...
    //tau ~ uniform(0,0.01);
     ...
     m ~ normal(1,1)T[0,];
     lam ~ normal(0,1)T[0,];
     gamma ~ normal(2,1)T[0,];
     beta ~ normal(1,1)T[0,];
     stay ~ normal(1,0.1)T[0,]; // we let stay be assumed to be close to 1 to help convergence
     alpha ~ normal(1,1)T[0,];
     RT ~ lba(k,I,m,lam,gamma,beta,stay,tau,alpha,x_r);
}
...
generated quantities {
     ...
     // here, have interesting rows
     // I predict response taking into account a minimum decision time of tau (2 seconds in the experiment, so tau=0.1)
     
     // pred_att1 = lba_rng([1.5, 1, 1, 1.5, 1.5, 0.85],k,I,m,lam,gamma,beta,stay,tau);
     // pred_att2 = lba_rng([1, 1.5, 1.5, 1, 0.85, 1.5],k,I,m,lam,gamma,beta,stay,tau);
     // pred_sim1 = lba_rng([1.5, 1, 1, 1.5, 0.95, 1.55],k,I,m,lam,gamma,beta,stay,tau);
     // pred_sim2 = lba_rng([1, 1.5, 1.5, 1, 1.55, 0.95],k,I,m,lam,gamma,beta,stay,tau);
     // pred_com1 = lba_rng([1.5, 1, 1, 1.5, 2, 0.5],k,I,m,lam,gamma,beta,stay,tau);
     // pred_com2 = lba_rng([1, 1.5, 1.5, 1, 0.5, 2],k,I,m,lam,gamma,beta,stay,tau);
}

// make additional models, with I,m,l,gamma different for second choice
// also mixed model with individual parameters
// explore better parameters
